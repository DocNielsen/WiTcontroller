...
{
  debug_print("displayUpdateFromWit(): keyapdeUseType ");
  debug_print(keypadUseType);
  debug_print(" menuIsShowing ");
  debug_print(menuIsShowing);
  debug_print(" multiThrottleIndex ");
  debug_print(multiThrottleIndex);
  debug_println("");
  if ((keypadUseType == KEYPAD_USE_OPERATION) && (!menuIsShowing) && (multiThrottleIndex == currentThrottleIndex))
  {
    writeOledSpeed();
  }
}
...
public:
  ...
  {
    debug_print("Received heartbeat. From: ");
    debug_print(heartBeatPeriod);
    debug_print(" To: ");
  ...
  void receivedServerDescription(String description)
  {
    ...
    debug_print("ServerType: ");
    debug_println(serverType);
  ...
  void receivedAlert(String message)
  {
    ...
    debug_println(message);
    if ((!message.equals("Connected")) && (!message.equals("Connecting..")) && (!message.equals("Steal from other WiThrottle or JMRI throttle Required")))
    {
      broadcastMessageText = String(message);
      broadcastMessageTime = millis();
      refreshOled();
  ...
  { // Vnnn
    debug_print("Received Speed: (");
    debug_print(millis());
    debug_print(") throttle: ");
    debug_print(multiThrottle);
    debug_print(" speed: ");
    debug_println(speed);
    ...
    if (currentSpeed[multiThrottleIndex] != speed)
    {
      ...
      if ((lastSpeedThrottleIndex != multiThrottleIndex) || ((millis() - lastSpeedSentTime) > 500))
      {
      ...
      {
        debug_print("Received Speed: skipping response: (");
        debug_print(millis());
        debug_print(") speed: ");
        debug_println(speed);
      }
  ...
  { // R{0,1}
    debug_print("Received Direction: ");
    debug_println(dir);
    int multiThrottleIndex = getMultiThrottleIndex(multiThrottle);
  ...
  { // R{0,1}
    debug_print("Received Direction: loco: ");
    debug_print(loco);
    debug_print(" Received Direction: ");
    debug_println(dir);
    // int multiThrottleIndex = getMultiThrottleIndex(multiThrottle);
  ...
  {
    debug_print("Received Fn: ");
    debug_print(func);
    debug_print(" State: ");
    debug_println((state) ? "True" : "False");
    int multiThrottleIndex = getMultiThrottleIndex(multiThrottle);
  ...
  void receivedRosterFunctionListMultiThrottle(char multiThrottle, String functions[MAX_FUNCTIONS])
  {
    ...
    for (int i = 0; i < MAX_FUNCTIONS; i++)
    {
      ...
      debug_print(i);
      debug_print(" - ");
      debug_println(functions[i]);
    }
  }
  void receivedTrackPower(TrackPower state)
  {
    debug_print("Received TrackPower: ");
    debug_println(state);
    if (trackPower != state)
    {
      trackPower = state;
  ...
  {
    debug_print("Received Roster Entries. Size: ");
    debug_println(size);
  ...
  {
    debug_print("Received Roster Entry, index: ");
    debug_print(index);
    debug_println(" - " + name);
    if (index < rosterSize)
    {
      ...
      { // got them all now.  and we need to sort them
        ...
        for (int i = 0; i < rosterSize; i++)
        {
          ...
          debug_print("Roster sorted: ");
          debug_print(rosterSortPointers[i]);
          debug_print(" | ");
          debug_println(rosterName[rosterSortedIndex[i]]);
        }
  ...
  {
    debug_print("Received Turnout Entries. Size: ");
    debug_println(size);
  ...
  {
    debug_print("Received Route Entries. Size: ");
    debug_println(size);
...
void enterSsidPassword()
{
  ...
  { // don't refresh the screen if nothing nothing has changed
...
{ // show the list from the specified values in config_network.h
  ...
  {
    debug_print(maxSsids);
    debug_println(MSG_SSIDS_LISTED);
    clearOledArray();
...
void witServiceLoop()
{
  ...
  if ((witConnectionState == CONNECTION_STATE_SELECTED) || (witConnectionState == CONNECTION_STATE_ENTERED))
  {
    connectWitServer();
  }
}
...
void browseWitService()
{
  ...
  {
    int j = 0;
    while ((noOfWitServices == 0) && ((nowTime - startTime) <= 10000))
    { // try for 10 seconds
  ...
  {
    debug_print(noOfWitServices);
    debug_println(MSG_SERVICES_FOUND);
    clearOledArray();
    ...
    for (int i = 0; i < foundWitServersCount; ++i)
    {
      ...
      debug_print(i);
      debug_print(": '");
      debug_print(foundWitServersNames[i]);
      debug_print("' (");
      debug_print(foundWitServersIPs[i]);
      debug_print(":");
      debug_print(foundWitServersPorts[i]);
      debug_println(")");
      if (i < 5)
      { // only have room for 5
...
void readPreferences()
{
  ...
  if (nvsInit)
  {
    ...
    for (int i = 0; i < MAX_THROTTLES; i++)
    {
      key[1] = '0' + i;
      for (int j = 0; j < 10; j++)
      { // assume a maximum of 10 locos per throttle
        key[2] = '0' + j;
        ...
        {
          debug_print("readPreferences(): Not Found - Key: ");
          debug_println(key);
        }
      }
    }
    ...
    for (int i = 0; i < maxThrottles; i++)
    {
      ...
      for (int j = 0; j < 10; j++)
      {
        ...
        if (j < wiThrottleProtocol.getNumberOfLocomotives(getMultiThrottleChar(i)))
        {
          ...
          debug_print("writePreferences(): Key: ");
          debug_print(key);
          debug_print(" - ");
          debug_println(locoNumber);
          count++;
        }
        else
        {
          ...
          {
            nvsPrefs.remove(key);
            debug_print("writePreferences(): Removed Key: ");
            debug_println(key);
          }
        }
      }
...
void rotary_onButtonClick()
{
  ...
  {
    if ((keypadUseType != KEYPAD_USE_SELECT_WITHROTTLE_SERVER) && (keypadUseType != KEYPAD_USE_ENTER_WITHROTTLE_SERVER) && (keypadUseType != KEYPAD_USE_SELECT_SSID) && (keypadUseType != KEYPAD_USE_SELECT_SSID_FROM_FOUND))
    {

      if ((millis() - rotaryEncoderButtonLastTimePressed) < rotaryEncoderButtonEncoderDebounceTime)
      { // ignore multiple press in that specified time
...
void rotary_loop()
{
  ...
  { // don't print anything unless value changed
    ...
    debug_print("Encoder From: ");
    debug_print(lastEncoderValue);
    debug_print(" to: ");
    debug_println(encoderValue);

    if ((millis() - rotaryEncoderButtonLastTimePressed) < rotaryEncoderButtonEncoderDebounceTime)
    { // ignore the encoder change if the button was pressed recently
    ...
    { // must have passed through zero
      ...
      debug_print("Corrected Encoder From: ");
      debug_print(lastEncoderValue);
      debug_print(" to: ");
      debug_println(encoderValue);
    }
...
void throttlePot_loop(bool forceRead)
{
  ...
  { // only ready it one every x seconds
    return;
  }
  ...
  if ((avgPotValue < lastThrottlePotValue - 5) || (avgPotValue > lastThrottlePotValue + 5) || (forceRead))
  {
    ...
    debug_print("Avg Pot Value: ");
    debug_println(avgPotValue);
    ...
    { // use notches
      ...
      for (int i = 0; i < 8; i++)
      {
        ...
        { /// Check to see if it is in notch i
    ...
    { // use a linear speed
      ...
      else if (newSpeed > 127)
      {
        newSpeed = 127;
      }
      int iSpeed = newSpeed;
      debug_print("newSpeed: ");
      debug_print(newSpeed);
      debug_print(" iSpeed: ");
      debug_println(iSpeed);
...
void batteryTest_loop()
{
  ...
  if (millis() - lastBatteryCheckTime > 10000)
  {
    ...
    debug_println(batteryTestValue);
...
{
  debug_print("keypadEvent((): ");
  debug_println(key);
  switch (keypad.getState())
  {
  case PRESSED:
    debug_print("Button ");
    debug_print(String(key - '0'));
...
void initialiseAdditionalButtons()
{
  ...
  for (int i = 0; i < maxAdditionalButtons; i++)
  {
    if (additionalButtonActions[i] != FUNCTION_NULL)
    {
      ...
      debug_print(i);
      debug_print(" pin:");
      debug_println(additionalButtonPin[i]);
...
void additionalButtonLoop()
{
  ...
  for (int i = 0; i < maxAdditionalButtons; i++)
  {
    if ((additionalButtonActions[i] != FUNCTION_NULL) && (additionalButtonPin[i] >= 0))
    {
      ...
      { // on process on a change
        if ((millis() - lastAdditionalButtonDebounceTime[i]) > additionalButtonDebounceDelay)
        { // only process if there is sufficent delay since the last read
          lastAdditionalButtonDebounceTime[i] = millis();
          additionalButtonRead[i] = buttonRead;

          if (((additionalButtonType[i] == INPUT_PULLUP) && (additionalButtonRead[i] == LOW)) || ((additionalButtonType[i] == INPUT) && (additionalButtonRead[i] == HIGH)))
          {
            debug_print("Additional Button Pressed: ");
            debug_print(i);
            debug_print(" pin:");
            debug_print(additionalButtonPin[i]);
            debug_print(" action:");
            debug_println(additionalButtonActions[i]);
            if (wiThrottleProtocol.getNumberOfLocomotives(currentThrottleIndexChar) > 0)
            { // only process if there are locos aquired
              doDirectAdditionalButtonCommand(i, true);
            }
            else
            { // check for actions not releted to a loco
          ...
          {
            debug_print("Additional Button Released: ");
            debug_print(i);
            debug_print(" pin:");
            debug_print(additionalButtonPin[i]);
            debug_print(" action:");
...
void loop()
{
  ...
  else
  {
    ...
    else
    {
      ...
      if ((lastServerResponseTime + (heartBeatPeriod * 4) < millis() / 1000) && (heartbeatCheckEnabled))
      {
        debug_print("Disconnected - Last:");
        debug_print(lastServerResponseTime);
        debug_print(" Current:");
...
{
  debug_print("doKeyPress(): key: ");
  debug_print(key);
  debug_print(" keypadUseType: ");
  debug_println(keypadUseType);
  ...
  { // pressed
    ...
    case KEYPAD_USE_OPERATION:
      debug_print("doKeyPress(): key operation... ");
      debug_println(key);
      ...
      case '#': // end of command
        debug_print("doKeyPress(): end of command... ");
        debug_print(key);
        debug_print("  :  ");
        debug_println(menuCommand);
      ...
      case '9':
      {
        debug_print("doKeyPress(): number... ");
        debug_print(key);
        debug_print("  cmd: '");
        debug_println(menuCommand);
        debug_print("' : ");
        debug_println(menuCharsRequired[menuCommand.substring(0, 1).toInt()]);

        int index0 = key - 48;
        int index1 = 0;
        if (menuCommand.length() >= 0)
          index1 = menuCommand.charAt(0) - 48;
        int index2 = 0;
        if (menuCommand.length() >= 1)
          index2 = menuCommand.charAt(1) - 48;
        ...
        { // append to the string
          ...
          if (((menuCommand.length() == 0) && (menuCharsRequired[index0] == MENU_ITEM_TYPE_DIRECT_COMMAND)) || ((menuCommand.length() == 1) && ((menuCharsRequired[index1] == MENU_ITEM_TYPE_SUB_MENU) || (menuCharsRequired[index1] == MENU_ITEM_TYPE_DIRECT_COMMAND))) || ((menuCommand.length() == 2) && (menuCharsRequired[index1] != MENU_ITEM_TYPE_ONE_OR_MORE_CHARS) && (menuCharsRequired[index2] == MENU_ITEM_TYPE_DIRECT_COMMAND)))
          {
          ...
          {
            if ((menuCharsRequired[index0] == MENU_ITEM_TYPE_SUB_MENU) && (menuCommand.length() == 0))
            { // menu type needs only one char
            ...
            { // menu type allows/requires more than one char
    ...
    case KEYPAD_USE_ENTER_WITHROTTLE_SERVER:
      debug_print("doKeyPress(): key: Enter wit... ");
      debug_println(key);
      switch (key)
      {
      case '0':
    ...
    case KEYPAD_USE_ENTER_SSID_PASSWORD:
      debug_print("doKeyPress(): key: Enter password... ");
      debug_println(key);
      switch (key)
      {
      case '0':
    ...
    case KEYPAD_USE_SELECT_WITHROTTLE_SERVER:
      debug_print("doKeyPress(): key: Select wit... ");
      debug_println(key);
      switch (key)
      {
      case '0':
    ...
    case KEYPAD_USE_SELECT_SSID:
      debug_print("doKeyPress(): key ssid... ");
      debug_println(key);
      switch (key)
      {
      case '0':
    ...
    case KEYPAD_USE_SELECT_SSID_FROM_FOUND:
      debug_print("doKeyPress(): key ssid from found... ");
      debug_println(key);
      switch (key)
      {
      case '0':
    ...
    case KEYPAD_USE_SELECT_ROSTER:
      debug_print("doKeyPress(): key Roster... ");
      debug_println(key);
      switch (key)
      {
      case '0':
    ...
    case KEYPAD_USE_SELECT_TURNOUTS_CLOSE:
      debug_print("doKeyPress(): key turnouts... ");
      debug_println(key);
      switch (key)
      {
      case '0':
    ...
    case KEYPAD_USE_SELECT_ROUTES:
      debug_print("doKeyPress(): key routes... ");
      debug_println(key);
      switch (key)
      {
      case '0':
    ...
    case KEYPAD_USE_SELECT_FUNCTION:
      debug_print("doKeyPress(): key function... ");
      debug_println(key);
      switch (key)
      {
      case '0':
  ...
  { // released
    ...
    case KEYPAD_USE_OPERATION:
      ...
      { // only process releases for the numeric keys + A,B,C,D and only if a menu command has not be started
...
{
  debug_print("doDirectCommand(): key: ");
  debug_println(key);
  int buttonAction = 0;
  if (key <= 57)
  {
  ...
  debug_print("doDirectCommand(): Action: ");
  debug_println(buttonAction);
  if (buttonAction != FUNCTION_NULL)
  {
    ...
    {
      if (pressed)
      { // only process these on the key press, not the release
...
{
  debug_print("doDirectAdditionalButtonCommand(): index: ");
  debug_println(buttonIndex);
...
{
  debug_print("doMenuCommand(): ");
  debug_print(menuCommand);
  debug_print(" : ");
  ...
  { // de-select loco
    ...
    { // not loco specified so release all
  ...
  { // toggle speed step additional Multiplier
    toggleAdditionalMultiplier();
    break;
  }
  ...
  { // throw point
    if (menuCommand.length() > startAt)
    {
      ...
      debug_print("throw point: ");
      debug_println(turnout);
  ...
  { // disable/enable the heartbeat Check
    toggleHeartbeatCheck();
    writeOledSpeed();
    break;
  }
  case MENU_ITEM_DROP_BEFORE_ACQUIRE_TOGGLE:
  { // disable/enable drop before Acquire
  ...
  { // increase number of Throttles
    changeNumberOfThrottles(true);
    break;
  }
  case MENU_ITEM_DECREASE_MAX_THROTTLES:
  { // decrease numbe rof throttles
    changeNumberOfThrottles(false);
    break;
  }
...
void resetMenu()
{
  ...
  if ((keypadUseType != KEYPAD_USE_SELECT_SSID) && (keypadUseType != KEYPAD_USE_SELECT_WITHROTTLE_SERVER))
  {
    keypadUseType = KEYPAD_USE_OPERATION;
  }
}
...
void resetFunctionLabels(int multiThrottleIndex)
{
  ...
  debug_println(multiThrottleIndex);
  for (int i = 0; i < MAX_FUNCTIONS; i++)
  {
...
String getLocoWithLength(String loco)
{
  ...
  if ((locoNo > SHORT_DCC_ADDRESS_LIMIT) || ((locoNo <= SHORT_DCC_ADDRESS_LIMIT) && (loco.charAt(0) == '0') && (!serverType.equals("DCC-EX"))))
  {
...
void toggleLocoFacing(int multiThrottleIndex, String loco)
{
  ...
  for (int i = 0; i < wiThrottleProtocol.getNumberOfLocomotives(multiThrottleIndexChar); i++)
  {
    if (wiThrottleProtocol.getLocomotiveAtPosition(multiThrottleIndexChar, i).equals(loco))
    {
      ...
      debug_print("toggleLocoFacing(): loco: ");
      debug_print(loco);
      debug_print(" current direction: ");
      debug_println(currentDirection);
...
String getDisplayLocoString(int multiThrottleIndex, int index)
{
  ...
  { // not the lead loco
    Direction leadLocoDirection = wiThrottleProtocol.getDirection(multiThrottleIndexChar,
...
void doDirectFunction(int multiThrottleIndex, int functionNumber, bool pressed, bool force)
{
  ...
  if (wiThrottleProtocol.getNumberOfLocomotives(multiThrottleIndexChar) > 0)
  {
    debug_print("direct fn: ");
    debug_print(functionNumber);
...
{
  char multiThrottleIndexChar = getMultiThrottleChar(multiThrottleIndex);
  debug_print("doFunction(): multiThrottleIndex ");
  debug_println(multiThrottleIndex);
  if (wiThrottleProtocol.getNumberOfLocomotives(multiThrottleIndexChar) > 0)
  {
    if (force)
...
void doFunctionWhichLocosInConsist(int multiThrottleIndex, int functionNumber, bool pressed, bool force)
{
  ...
  { // at the momemnt the only other option in CONSIST_ALL_LOCOS
    wiThrottleProtocol.setFunction(multiThrottleIndexChar, "*", functionNumber, pressed, force);
  }
  debug_print("doFunctionWhichLocosInConsist(): fn: ");
  debug_print(functionNumber);
  debug_println(" Released");
}
...
void changeNumberOfThrottles(bool increase)
{
  if (increase)
  {
    maxThrottles++;
    if (maxThrottles > 6)
      maxThrottles = 6; /// can't have more than 6
  }
  else
  {
    maxThrottles--;
...
void stopThenToggleDirection()
{
  if (wiThrottleProtocol.getNumberOfLocomotives(currentThrottleIndexChar) > 0)
  {
    ...
    {
      if (toggleDirectionOnEncoderButtonPressWhenStationary)
        toggleDirection(currentThrottleIndex);
    }
    currentSpeed[currentThrottleIndex] = 0;
  }
...
{
  debug_print("selectEditConsistList() ");
  debug_println(selection);
...
void receivingServerInfoOled(int index, int maxExpected)
{
  ...
  if (index < (maxExpected - 1))
  {
    ...
    { // refresh it every X seconds if needed
...
void writeOledBattery()
{
  ...
  if ((useBatteryTest) && (showBatteryTest != NONE) && (lastBatteryCheckTime > 0))
  {
    ...
    if (lastBatteryTestValue > 10)
      u8g2.drawLine(x + 1, y - 6, x + 1, y - 3);
    if (lastBatteryTestValue > 25)
      u8g2.drawLine(x + 2, y - 6, x + 2, y - 3);
    if (lastBatteryTestValue > 50)
      u8g2.drawLine(x + 3, y - 6, x + 3, y - 3);
    if (lastBatteryTestValue > 75)
      u8g2.drawLine(x + 4, y - 6, x + 4, y - 3);
    if (lastBatteryTestValue > 90)
      u8g2.drawLine(x + 5, y - 6, x + 5, y - 3);
